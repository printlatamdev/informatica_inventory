...
<head>
    ...
    <title>Equipos Nas - COLOR DIGITAL</title>
    <link rel="stylesheet" href="styles_equipos.css">
</head>
<body>
    <div class="container">
        <h1>EQUIPOS NAS DE COLOR DIGITAL</h1>
        ...
        <?php
            ...
            if (!$obj_conexion) {
               ...
               $total_query = "SELECT COUNT(*) as total FROM equipo WHERE tipo_equipo='Nas' 
               ...
               $var_consulta = "SELECT * FROM equipo WHERE tipo_equipo='Nas' 
                                AND (tipo_equipo LIKE '%$search%' OR 
                                     nombre_host LIKE '%$search%' OR 
                                     num_serie LIKE '%$search%' OR 
                                     num_inventario LIKE '%$search%' OR 
               ...
               if ($var_resultado->num_rows > 0) {
                   ...
                   while ($var_fila = $var_resultado->fetch_array()) {
                       echo "<tr>
                             ...
                             <td>{$var_fila['ip']}</td>
                             <td><a href='manto_nas.php?id={$var_fila['id']}' class='button maintenance-button'>Mantenimiento</a></td>
                             </tr>";
                   }
                   echo "</table></div>";
