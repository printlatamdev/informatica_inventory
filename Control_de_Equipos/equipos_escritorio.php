...
<head>
    ...
    <title>Equipos de Escritorio - COLOR DIGITAL</title>
    <link rel="stylesheet" href="styles_equipos.css">
</head>
<body>
    <div class="container">
        <h1>EQUIPOS DE ESCRITORIO DEL COLOR DIGITAL</h1>
        ...
        <?php
            ...
            if (!$obj_conexion) {
                ...
                $total_result_query = "SELECT COUNT(*) as total FROM equipo WHERE tipo_equipo='Escritorio' 
                                       AND (tipo_equipo LIKE '%$search%' OR 
                                            ...
                                            ip LIKE '%$search%')";
                $total_result = $obj_conexion->query($total_result_query);
                $total_rows = $total_result->fetch_assoc()['total'];
                $total_pages = ceil($total_rows / $limit);

                $var_consulta = "SELECT * FROM equipo WHERE tipo_equipo='Escritorio' 
                                 AND (tipo_equipo LIKE '%$search%' OR 
                                      nombre_host LIKE '%$search%' OR 
                                      num_serie LIKE '%$search%' OR 
                                      num_inventario LIKE '%$search%' OR 
                ...
                if ($var_resultado->num_rows > 0) {
                    ...
                    while ($var_fila = $var_resultado->fetch_array()) {
                        echo "<tr>
                              ...
                              <td>{$var_fila['ip']}</td>
                              <td><a href='manto_escritorio.php?id={$var_fila['id']}' class='button maintenance-button'>Mantenimiento</a></td>
                              </tr>";
                    }
                    echo "</table></div>";
